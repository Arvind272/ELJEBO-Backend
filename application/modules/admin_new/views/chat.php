        ...
        <div class="tcol w-xl bg-tr-white lt b-r">
            ...
            <div class="p-15 collapse collapse-xs collapse-sm" id="open-chats">
                ...
                <ul class="list-unstyled" id="inbox">
                    ...
                    if (!empty($chat_data) && is_array($chat_data)) {
                        $i = 1;
                        foreach ($chat_data as $key => $chatdata) {

                            $room_id        = $chatdata->room_id;
                            $profile_pic    = $chatdata->image;
                            if($chatdata->user_type == 1){
                                $path = GET_CUSTOMER_IMG.$chatdata->id.'/'.$profile_pic;
                            }else{
                                $path = GET_IMG_SERVICE_PROVIDER.$chatdata->id.'/'.$profile_pic;
                            }
                            $profile_pic    = $path;

                            $name           = ucwords($chatdata->firstname.' '.$chatdata->lastname);
                            $user_id        = $chatdata->message_from;
                            $active_class = ($i == 1) ? 'active' : '';
                            ?>
                            <li class="<?php echo $active_class; ?>">
                                <a href="javascript:void(0);" onclick="return loadChatData('<?php echo $room_id ?>', '<?php echo $name; ?>','<?php echo $user_id; ?>', '<?php echo $profile_pic; ?>');" class="<?php echo $active_class; ?>">
                                    <div class="media">
                                        <div class="media-left thumb thumb-sm">
                                            <?php if (dcrf_image_exist($chatdata->profile_pic)) : ?>
                                                <img class="media-object img-circle" src="<?php echo $profile_pic; ?>">
                                                <?php else: ?>
...
<script>
    ...
    function loadChatData(room_id, name='', sender_id='',profile_pic = ''){
        ...
        urlRef.once("value", function(snapshot) {
            ...
            snapshot.forEach(function(child) {
                ...
                if(child.val().sender_id == sender_id){

                    html += '<li class="out"><div class="media"><div class="pull-right thumb thumb-sm">';
                    if(child.val().url != ''){
                        html += '<img class="media-object img-circle chat_to_propic" src="'+child.val().url+'" alt="">';
                    }else{
                        html += '<img class="media-object img-circle chat_to_propic" src="'+profile_pic+'" alt="">';
                    }
                    html += '</div><div class="media-body"><p class="media-heading"><a href="javascript:void(0);" class="name">'+child.val().name+'</a><span class="datetime"></span></p><span class="body">'+child.val().text+'</span></div></div></li>';
